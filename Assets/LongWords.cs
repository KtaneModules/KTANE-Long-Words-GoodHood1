...
public class LongWords : MonoBehaviour
{
    ...
    public KMSelectable[] DirectionButtons; //Up, Down, Left, Right
    public KMSelectable SubmitButton;

    private string[] possibleWords = { "PNEUMONOULTRAMICROSCOPICSILICOVOLCANOCONIOSIS", "PSEUDOPSEUDOHYPOPARATHYROIDISM", 
    ...
    void Awake()
    {
        ...
        foreach (KMSelectable button in DirectionButtons) {
            button.OnInteract += delegate () { DirectionPress(button); return false; };
        }
    ...
    void Start()
    {
        ...
        DetermineWord();
        DetermineFirstLetterRanges();

        Debug.LogFormat("[Long Words #{0}] The corrent long word is {1}.", ModuleId, chosenWord);
        Debug.LogFormat("[Long Words #{0}] The correct range for the first letter was {1}-{2} (inclusive).", ModuleId, firstLetterRanges[0], firstLetterRanges[1]);

        DetermineWordSection();

        chosenSixLetterWord = SixLetterWords[Rnd.Range(0, SixLetterWords.Count())];
        displayTexts[0].text = chosenSixLetterWord;
        encryptedWord = EncryptPlayfair(wordSection, chosenSixLetterWord);
        displayTexts[2].text = encryptedWord;

    }       

    void DirectionPress(KMSelectable btn)
    {
        btn.AddInteractionPunch(0.1f);
        Audio.PlayGameSoundAtTransform(KMSoundOverride.SoundEffect.ButtonPress, btn.transform);
        if (ModuleSolved || isAnimating) return;
        switch (Array.IndexOf(DirectionButtons, btn))
        {
            case 0: //UP
                currentWordIndex--;
    ...
    void SubmitPress()
    {
        ...
        if (ModuleSolved || isAnimating) return;
        string currentDisplay = displayTexts[1].text;
        if (encryptedWord == EncryptPlayfair(currentDisplay, chosenSixLetterWord))
        {
            Debug.LogFormat("[Long Words #{0}] You submitted the word section of {1} which was a correct key. Module Solved!", ModuleId, currentDisplay);
            StartCoroutine(SolveReveal());
            ModuleSolved = true;
        }
        else
        {
            StartCoroutine(StrikeAnim());
            Debug.LogFormat("[Long Words #{0}] You submitted the word section of {1} and {2} encrypted to {3} which is not correct. Strike!", ModuleId, currentDisplay, chosenSixLetterWord,EncryptPlayfair(currentDisplay, chosenSixLetterWord) );
        }
    }
    
    IEnumerator SolveReveal()
    {
        isAnimating = true;
        for (int i = 0; i < 6; i++)
        {
            yield return new WaitForSeconds(0.3f);
            displayTexts[0].text = encryptedWord.Substring(0, i + 1) + chosenSixLetterWord.Substring(i + 1, 5 - i);
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            displayTexts[2].color = Color.white;
        }
        yield return new WaitForSeconds(0.12f);
        foreach (TextMesh text in displayTexts)
        {
    ...
    {
        isAnimating = true;
        string submittedEncrypted = EncryptPlayfair(displayTexts[1].text, displayTexts[0].text);
        for (int i = 0; i < 6; i++)
        {
            yield return new WaitForSeconds(0.3f);
            displayTexts[0].text = submittedEncrypted.Substring(0, i + 1) + chosenSixLetterWord.Substring(i + 1, 5 - i);
            Audio.PlaySoundAtTransform("Bip", transform);
        }
        for (int i = 0; i < 2; i++)
        {
            ...
            displayTexts[2].color = Color.red;
            yield return new WaitForSeconds(0.12f);
            displayTexts[0].color = Color.white;
            displayTexts[2].color = Color.white;
        }
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            displayTexts[0].text = "??????".Substring(0, i + 1) + submittedEncrypted.Substring(i + 1, 5 - i);
            displayTexts[2].text = "??????".Substring(0, i + 1) + encryptedWord.Substring(i + 1, 5 - i);
        }
        for (int i = 0; i < 6; i++)
        {
            yield return new WaitForSeconds(0.1f);
            displayTexts[0].text = chosenSixLetterWord.Substring(0, i + 1) + "??????".Substring(i + 1, 5 - i);
            displayTexts[2].text = encryptedWord.Substring(0, i + 1) + "??????".Substring(i + 1, 5 - i);
        }
        yield return new WaitForSeconds(0.3f);
        displayTexts[0].color = Color.white;
        displayTexts[2].color = Color.white;
    ...
    string EncryptPlayfair(string keySection, string encrypted)
    {
        ...
        for (int pair = 0; pair < encryptedList.Count(); pair += 2)
        {
            int[] c1Coords = { (int)Math.Floor(key.IndexOf(encryptedList[pair]) / 5.0), key.IndexOf(encryptedList[pair]) % 5};
            int[] c2Coords = { (int)Math.Floor(key.IndexOf(encryptedList[pair+1]) / 5.0), key.IndexOf(encryptedList[pair+1]) % 5 };
            int[] c1NewCoords = c1Coords.ToArray();
            int[] c2NewCoords = c2Coords.ToArray();

            if (c1Coords[0] == c2Coords[0])
            {
                c1NewCoords[1] = (c1Coords[1] + 1) % 5;
                c2NewCoords[1] = (c2Coords[1] + 1) % 5;
            }
            else if (c1Coords[1] == c2Coords[1])
            {
                c1NewCoords[0] = (c1Coords[0] + 1) % 5;
                c2NewCoords[0] = (c2Coords[0] + 1) % 5;
            }
            else
            {
                c1NewCoords[1] = c2Coords[1];
                c2NewCoords[1] = c1Coords[1];
            }

            EncryptedMessage += key[c1NewCoords[0] * 5 + c1NewCoords[1]];
            EncryptedMessage += key[c2NewCoords[0] * 5 + c2NewCoords[1]];
        }
    ...
    void DetermineWord()
    {
        if (Bomb.GetBatteryCount() > 6) chosenWord = possibleWords[0];
        else if ((Bomb.GetPortPlates().Where(p => p.Length == 0)).Count() >= 2) chosenWord = possibleWords[1];
        else if (Bomb.GetSerialNumberLetters().Count("AEIOU".Contains) >= 2) chosenWord = possibleWords[2];
        else if (Math.Abs(Bomb.GetOnIndicators().Count() - Bomb.GetOffIndicators().Count()) >= 2) chosenWord = possibleWords[3];
        else if (Bomb.GetSerialNumberNumbers().Contains(2)) chosenWord = possibleWords[4];
        else chosenWord = possibleWords[5];

    }

    void DetermineFirstLetterRanges()
    {
        firstLetterRanges[0] = (Bomb.GetBatteryCount() + 1) * (Bomb.GetBatteryHolderCount() + 1) + (Bomb.GetPortCount() + 1) * (Bomb.GetPortPlateCount() + 1);
        
        int serialNumbersProduct = 1;
        foreach (int digit in Bomb.GetSerialNumberNumbers())
            serialNumbersProduct *= digit == 0 ? 10 : digit;
        int serialLettersProduct = 1;
        foreach (char letter in Bomb.GetSerialNumberLetters())
            serialLettersProduct *= "ABCDEFGHIJKLMNOPQRSTUVWXYZ".IndexOf(letter) + 1;

        firstLetterRanges[1] = serialNumbersProduct - serialLettersProduct + 1;

        for (int i = 0; i < firstLetterRanges.Length; i++)
        {
            firstLetterRanges[i] = Math.Abs(firstLetterRanges[i]);
            if (firstLetterRanges[i] + 1 > chosenWord.Length)
                firstLetterRanges[i] = (firstLetterRanges[i] % chosenWord.Length) + 1;
        }
    }
    ...
    void DetermineWordSection()
    {
        ...
        else
        {
            ...
            for (int i = firstLetterRanges[0]; i < chosenWord.Length; i++)
                possibleIndexes.Add(i);
            for (int i = 0; i < firstLetterRanges[1]; i++)
                possibleIndexes.Add(i);
    ...
    {
        currentWordIndex = currentWordIndex < 0 ? currentWordIndex + possibleWords.Length : currentWordIndex % possibleWords.Length;
        currentLetterIndex = currentLetterIndex < 0 ? currentLetterIndex + possibleWords[currentWordIndex].Length : currentLetterIndex % possibleWords[currentWordIndex].Length;
        if (currentLetterIndex + 5 > possibleWords[currentWordIndex].Length) 
        {
            string endText = possibleWords[currentWordIndex].Substring(currentLetterIndex);
            displayTexts[1].text = endText + possibleWords[currentWordIndex].Substring(0, 5 - endText.Length);
        } 
        else displayTexts[1].text = possibleWords[currentWordIndex].Substring(currentLetterIndex, 5);
